...
class ProductionPromptProcessor:
    ...
    def build_master_analyst_prompt(self, market_data: Dict[str, Any]) -> str:
        ...
        template_data = {
            ...
            'support_level': market_data.get('support', market_data.get('price', 100) * 0.95),
            'resistance_level': market_data.get('resistance', market_data.get('price', 100) * 1.05),
...
class TradingSignalSimulator:
    ...
    def __init__(self, ollama_client: OllamaClient, prompt_processor: ProductionPromptProcessor):
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                decision = {
                    ...
                    'position_size': max(0.01, min(0.2, parsed.get('position_size_suggestion', 0.05))),
...
class TestOllamaWithPrompts:
    ...
    async def test_multiple_stock_analysis(self, trading_simulator, sample_stocks):
        ...
        for stock_data in sample_stocks:
            try:
                ...
                print(f"{stock_data['ticker']}: {decision['decision']} ({decision['confidence']}%)")
        ...
        print(f"Decision distribution: {dict((d, decisions.count(d)) for d in unique_decisions)}")

        # Performance summary
        avg_confidence = sum(result[1]['confidence'] for result in results) / len(results)
        avg_processing_time = sum(result[1]['processing_time'] for result in results) / len(results)
    ...
    async def test_scenario_analysis(self, trading_simulator):
        ...
        for scenario in scenarios:
            print(f"\n{scenario['name']}:")
            print(f"RSI: {scenario['data']['rsi']}, Change: {scenario['data']['daily_change']}%")
    ...
    async def test_production_data_integration(self, trading_simulator):
        ...
        parquet_path = Path(__file__).parent.parent.parent / "data" / "parquet" / "market_data"
        ...
        print(f"Analyzing real data: {market_data['ticker']} at ${market_data['price']}")
    ...
    async def test_performance_benchmarking(self, trading_simulator, sample_stocks):
        ...
        avg_processing_time = sum(d['processing_time'] for d in decisions) / total_decisions

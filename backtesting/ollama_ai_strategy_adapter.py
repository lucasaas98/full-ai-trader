...
from backtest_models import SignalType, Signal
...
class OllamaAIStrategyAdapter:
    ...
    def __init__(self,
        ...
        self.ollama_url = ollama_url or os.getenv('OLLAMA_URL', 'http://192.168.1.133:11434')
        ...
        logger.info(f"Ollama AI Strategy initialized: {self.ollama_url} using {self.ollama_model}")

    def _load_prompts_config(self, config_path: str = None) -> Dict[str, Any]:
        """Load prompts configuration from YAML file."""
        if config_path is None:
            config_path = Path(__file__).parent.parent / "config" / "ai_strategy" / "prompts.yaml"
    ...
    async def generate_signal(self,
                            ...
                            current_data,
                            historical_data,
                            market_context) -> Optional[Signal]:
        """
        ...
        try:
            ...
            prompt = self._build_analysis_prompt(symbol, current_data, historical_data, market_context)
            ...
            if signal:
                ...
                logger.debug(f"Generated {signal.action.value} signal for {symbol} "
    ...
    def _build_analysis_prompt(self,
        ...
        current_price = getattr(current_data, 'close', 0) if hasattr(current_data, 'close') else current_data.get('close', 0)
        current_price = float(current_price)  # Ensure float for calculations
        volume = getattr(current_data, 'volume', 0) if hasattr(current_data, 'volume') else current_data.get('volume', 0)
        volume = float(volume)  # Ensure float for calculations

        # Calculate technical indicators from historical data
        tech_indicators = self._calculate_technical_indicators(historical_data, current_data)
    ...
    def _build_fallback_prompt(self, symbol: str, current_data, historical_data) -> str:
        ...
        rsi = self._simple_rsi_calculation(historical_data) if len(historical_data) > 14 else 50
...
"""
    ...
    def _calculate_technical_indicators(self, historical_data, current_data) -> Dict[str, float]:
        """Calculate technical indicators from historical data."""
        indicators = {}
        ...
        try:
            ...
            for data_point in historical_data:
                if hasattr(data_point, 'close'):
                    ...
                    volumes.append(float(data_point.volume) if hasattr(data_point, 'volume') else 0)
                else:
                    prices.append(float(data_point.get('close', 0)))
                    volumes.append(float(data_point.get('volume', 0)))

            current_price = getattr(current_data, 'close', 0) if hasattr(current_data, 'close') else current_data.get('close', 0)
            current_price = float(current_price)  # Ensure current_price is float for calculations
        ...
        try:
            ...
            for i, candle in enumerate(recent):
                ...
                if close_price > open_price:
                    descriptions.append(f"Day {i+1}: Green candle (+{((close_price-open_price)/open_price)*100:.1f}%)")
                elif close_price < open_price:
                    descriptions.append(f"Day {i+1}: Red candle ({((close_price-open_price)/open_price)*100:.1f}%)")
                else:
    ...
    def _identify_technical_patterns(self, indicators: Dict[str, float]) -> str:
        ...
        return "; ".join(patterns) if patterns else "No clear technical patterns identified"
...
- Volatility: {'High' if vix_level > 25 else 'Low' if vix_level < 15 else 'Normal'}"""
    ...
    def _parse_ai_response(self, response: OllamaResponse, symbol: str, current_data) -> Optional[Signal]:
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                return Signal(
                    ...
                    position_size=min(parsed.get('position_size_suggestion', 0.05), self.max_position_size),
                    reasoning=reasoning,
                    metadata={
    ...
    def _parse_text_response(self, content: str, symbol: str, current_data) -> Optional[Signal]:
    ...
    def __str__(self):
        return f"OllamaAIStrategyAdapter(model={self.ollama_model}, url={self.ollama_url})"

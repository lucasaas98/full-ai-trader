...
from backtest_models import (
    MarketData, SignalType, TimeFrame, Signal,
    BacktestPosition, BacktestTrade, BacktestResults,
    FinVizData, TechnicalIndicators
)
...
class RealBacktestConfig:
    ...
    screener_types: List[str] = field(default_factory=lambda: ["momentum", "breakouts", "value_stocks"])
...
class HistoricalDataFeeder:
    ...
    async def get_market_data_for_date(self, date: datetime, symbols: List[str]) -> Dict[str, MarketData]:
        ...
        for symbol in symbols:
            try:
                ...
                self.logger.warning(f"Could not load data for {symbol} on {date.date()}: {e}")
    ...
    async def get_screener_data_for_date(self, date: datetime) -> Dict[str, List[str]]:
        ...
        for screener_type in self.config.screener_types:
            try:
                ...
                self.logger.warning(f"Could not load screener data for {screener_type} on {date.date()}: {e}")
...
class MockAIStrategy:
    ...
    async def generate_signal(self, symbol: str, current_data, historical_data, market_context):
...
class RealBacktestEngine:
    ...
    async def run_backtest(self) -> BacktestResults:
        ...
        self.logger.info(f"Starting real backtest from {self.config.start_date.date()} to {self.config.end_date.date()}")
    ...
    async def _process_trading_day(self, date: datetime, day_index: int, total_days: int) -> None:
        ...
        try:
            ...
            if self.ai_strategy and len(self.positions) < self.config.max_positions:
                ...
                for signal in signals:
                    await self._execute_signal(signal, market_data.get(signal.symbol), date)

            # Log progress
            if (day_index + 1) % 50 == 0 or day_index == 0 or day_index == total_days - 1:
                portfolio_value = await self._calculate_portfolio_value(date)
                self.logger.info(f"Progress: {day_index+1}/{total_days} days, Portfolio: ${portfolio_value:,.2f}")
    ...
    async def _update_position_prices(self, market_data: Dict[str, MarketData], date: datetime) -> None:
        """Update current prices for existing positions."""
        for symbol, position in self.positions.items():
            if symbol in market_data:
                position.current_price = market_data[symbol].close
                position.unrealized_pnl = (position.current_price - position.entry_price) * Decimal(str(position.quantity))

    async def _check_exit_conditions(self, market_data: Dict[str, MarketData], date: datetime) -> None:
    ...
    async def _generate_ai_signals(self, market_data: Dict[str, MarketData], date: datetime) -> List[Signal]:
        ...
        try:
            ...
            for symbol, data in market_data.items():
                ...
                historical_data = await self._get_historical_data_for_analysis(symbol, date)
    ...
    async def _get_historical_data_for_analysis(self, symbol: str, current_date: datetime) -> Optional[List[MarketData]]:
    ...
    async def _execute_signal(self, signal: Signal, market_data: MarketData, date: datetime) -> None:
        ...
        try:
            ...
            if required_cash > self.cash:
                self.logger.warning(f"Insufficient cash for {symbol}: need ${required_cash}, have ${self.cash}")
                return

            # Create position
            quantity = position_size if signal.action == SignalType.BUY else -position_size
    ...
    async def _close_position(self, symbol: str, market_data: MarketData, date: datetime, reason: str) -> None:
        ...
        try:
            ...
            # Calculate commission
            commission = await self._calculate_commission(exit_price, abs(position.quantity))

            # Calculate P&L
            if position.is_long:
                pnl = (exit_price - position.entry_price) * Decimal(str(abs(position.quantity)))
            else:
                pnl = (position.entry_price - exit_price) * Decimal(str(abs(position.quantity)))

            pnl -= commission  # Subtract commission
            pnl_percentage = float(pnl / (position.entry_price * Decimal(str(abs(position.quantity)))))
    ...
    async def _calculate_position_size(self, signal: Signal, market_data: MarketData) -> int:
        ...
        try:
            ...
            # Calculate position size
            position_value = available_cash * Decimal('0.95')  # Leave some buffer for commission
    ...
    async def _calculate_entry_price(self, market_data: MarketData, action: SignalType) -> Decimal:
        """Calculate entry price with slippage."""
    ...
    async def _calculate_exit_price(self, market_data: MarketData, is_long: bool) -> Decimal:
        """Calculate exit price with slippage."""
        base_price = market_data.close
        slippage_factor = self.config.slippage_bps / Decimal('10000')
    ...
    async def _calculate_commission(self, price: Decimal, quantity: int) -> Decimal:
        ...
        percentage_commission = price * Decimal(str(quantity)) * self.config.commission_percentage
    ...
    async def _generate_results(self, execution_time: float) -> BacktestResults:
        ...
        try:
            ...
            avg_win = float(sum(t.pnl for t in winning_trades) / len(winning_trades)) if winning_trades else 0
            avg_loss = float(sum(t.pnl for t in losing_trades) / len(losing_trades)) if losing_trades else 0
            ...
            if self.portfolio_history:
                ...
                if returns:
                    ...
                    annualized_return = float((1 + np.mean(returns)) ** 252 - 1)  # Assuming daily returns

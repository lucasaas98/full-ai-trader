...
from shared.models import TradeSignal, Position, SignalType
    ...
    class MockStrategyEngineService:
        ...
        async def check_portfolio_risk(self, signal, portfolio):
            ...
            if existing_position:
                total_exposure = float(existing_position.market_value) + (float(signal.price) * signal.quantity)
...
def momentum_strategy():
    ...
    from services.strategy_engine.src.base_strategy import BaseStrategy, StrategyConfig, StrategyMode, Signal
...
class TestStrategyEngineIntegration:
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_strategy_execution(self, strategy_engine, momentum_strategy, sample_historical_data):
    ...
    async def test_strategy_risk_integration(self, strategy_engine, sample_portfolio):
        ...
        risk_result = await strategy_engine.check_portfolio_risk(large_signal, sample_portfolio)
        ...
        adjusted_size = await strategy_engine.calculate_position_size(large_signal, sample_portfolio)
...
class TestStrategyBacktesting:
    ...
    def backtester(self):
        ...
        class MockStrategyBacktester:
            ...
            def apply_slippage_and_commission(self, symbol, side, intended_price, quantity):
                ...
                if side == "buy":
                    executed_price = intended_price + slippage_amount + commission_amount / quantity
                else:
                    executed_price = intended_price - slippage_amount - commission_amount / quantity
    ...
    @pytest.mark.unit
    def test_backtest_execution(self, backtester, sample_historical_data, momentum_strategy):
...
class TestSignalProcessor:
    ...
    def signal_processor(self, mock_redis_client):
        ...
        class MockSignalProcessor:
            ...
            async def persist_signal(self, signal):
                ...
                self.redis_client.hset(f"signals:{signal.symbol}", signal.id, "signal_data")
                return {"id": signal.id, "status": "persisted"}

            async def retrieve_signals(self, symbol=None, start_time=None, end_time=None):
    ...
    async def test_signal_aggregation(self, signal_processor, trading_signal_factory):
        ...
        signals = [
            trading_signal_factory(strategy_name="momentum", strength=0.8, signal_type="BUY"),
            trading_signal_factory(strategy_name="rsi", strength=0.7, signal_type="BUY"),
            trading_signal_factory(strategy_name="macd", strength=0.6, signal_type="BUY")
        ]
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_conflicting_signals_handling(self, signal_processor, trading_signal_factory):
        """Test handling of conflicting signals"""

        signals = [
            trading_signal_factory(strategy_name="momentum", strength=0.8, signal_type="BUY"),
            trading_signal_factory(strategy_name="mean_reversion", strength=0.9, signal_type="SELL")
        ]
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_signal_persistence(self, signal_processor, mock_redis_client, sample_trading_signal):
    ...
    def test_signal_filtering(self, signal_processor, trading_signal_factory):
        ...
        filtered_signals = signal_processor.filter_signals_by_strength(signals, min_strength=0.5)
...
class TestRiskIntegration:
    ...
    async def test_position_size_calculation(self, strategy_engine, sample_portfolio):
        ...
        position_size = await strategy_engine.calculate_position_size(signal, sample_portfolio)
    ...
    async def test_portfolio_risk_check(self, strategy_engine, sample_portfolio):
        ...
        risk_check = await strategy_engine.check_portfolio_risk(high_risk_signal, sample_portfolio)
    ...
    async def test_correlation_risk_check(self, strategy_engine):
        ...
        with patch.object(strategy_engine, 'get_correlation_matrix') as mock_correlation:

...
class TestTechnicalIndicators:
    ...
    def sample_price_data(self):
        ...
        for i, close in enumerate(prices[1:]):
            ...
            data.append({
                'timestamp': datetime.now(timezone.utc) - timedelta(days=n_points-i-1),
                'open': open_price,
                'high': high,
                'low': low,
                'close': close,
    ...
    def test_rsi_calculation(self, sample_price_data):
        ...
        valid_rsi = [val for val in valid_rsi if not (isinstance(val, float) and val != val)]
    ...
    def test_bollinger_bands_calculation(self, sample_price_data):
        ...
        result = TechnicalIndicators.bollinger_bands(sample_price_data, period=20, std_dev=2)
        ...
        if len(valid_data) > 0:
            ...
            for row in valid_data.select(['close', 'bb_lower', 'bb_middle', 'bb_upper']).rows():
    ...
    def test_stochastic_calculation(self, sample_price_data):
        ...
        result = TechnicalIndicators.stochastic(sample_price_data, k_period=14, d_period=3)

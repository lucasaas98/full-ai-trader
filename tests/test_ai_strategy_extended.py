...
from services.strategy_engine.src.ai_models import (
    AIDecisionRecord, AIPerformanceMetrics, MarketRegimeState,
    AIContext, PromptContext, ConsensusResult, PerformanceReport,
    create_performance_summary, decision_to_dict
)
...
class TestResponseCache:
    ...
    async def test_cache_expiration(self, cache):
        ...
        with patch('services.strategy_engine.src.ai_strategy.datetime') as mock_datetime:
...
class TestDataContextBuilder:
    ...
    def test_rsi_extremes(self, complex_data):
        ...
        # All up moves
        up_data = complex_data.with_columns(
    ...
    def test_support_resistance_detection(self, complex_data):
        ...
        support, resistance = DataContextBuilder._calculate_support_resistance(complex_data)
...
class TestAIStrategyEngineIntegration:
    ...
    def strategy_engine(self, mock_anthropic_client):
        ...
        with patch('services.strategy_engine.src.ai_strategy.AnthropicClient') as mock_client_class:
    ...
    async def test_analyze_with_cache_hit(self, strategy_engine):
        ...
        with patch.object(strategy_engine.consensus_engine, 'build_consensus') as mock_consensus:
    ...
    async def test_exit_strategy_triggers(self):
        ...
        with patch.dict(os.environ, {'ANTHROPIC_API_KEY': 'test_key'}):
            with patch('services.strategy_engine.src.ai_integration.create_async_engine'):
                ...
                integration.price_data_buffer['TEST'] = integration.price_data_buffer['TEST'].with_columns(
                    pl.lit(94.0).alias('close')
                )

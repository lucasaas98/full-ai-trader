...
from sqlalchemy import text, MetaData, Table, Column, Integer, String, DateTime, Numeric, Boolean, Text, inspect
...
class SharedDatabaseManager:
    ...
    async def get_portfolio_snapshots(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
    ...
    async def get_risk_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session_factory() as session:
                ...
                trade_result = await session.execute(trade_query, {'start_date': start_date})
                ...
                risk_result = await session.execute(risk_query, {'start_date': start_date})
                risk_events = {}
                ...
                stats = {
                    ...
                    'win_rate': (trade_row.winning_trades / max(trade_row.total_trades, 1)) if trade_row.total_trades else 0,
        ...
        try:
            async with self.session_factory() as session:
                ...
                await session.execute(query, {
                    ...
                    'pattern_day_trader': snapshot_data.get('pattern_day_trader', False),

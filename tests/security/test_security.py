...
class TestAuthentication:
    ...
    def test_api_key_validation(self, api_key_manager):
        ...
        assert not api_key_manager.validate_api_key(invalid_key), "Invalid API key accepted"
    ...
    def test_api_key_expiration(self, api_key_manager):
        ...
        assert not api_key_manager.validate_api_key(expired_key), "Expired API key accepted"
        ...
        assert api_key_manager.validate_api_key(short_lived_key), "Valid short-lived key rejected"
    ...
    def test_jwt_token_validation(self, security_manager):
        ...
        assert security_manager.validate_jwt_token(valid_token), "Valid JWT token rejected"

        # Expired token
        expired_token = SecurityTestHelper.create_test_jwt_token(payload, expired=True)
        assert not security_manager.validate_jwt_token(expired_token), "Expired JWT token accepted"

        # Malformed token
        malformed_token = "not.a.jwt.token"
        assert not security_manager.validate_jwt_token(malformed_token), "Malformed JWT token accepted"

        # Tampered token
        tampered_token = valid_token[:-5] + "XXXXX"
        assert not security_manager.validate_jwt_token(tampered_token), "Tampered JWT token accepted"
    ...
    def test_password_strength_validation(self, security_manager):
        ...
        for weak_password in weak_passwords:
            assert not security_manager.is_password_strong(weak_password), \
        ...
        for strong_password in strong_passwords:
            assert security_manager.is_password_strong(strong_password), \
    ...
    def test_session_management(self, security_manager):
        ...
        # Validate session
        assert security_manager.validate_session(session_token), "Valid session rejected"

        # Test session expiry
        with patch('time.time', return_value=time.time() + 7200):  # 2 hours later
            assert not security_manager.validate_session(session_token), "Expired session accepted"

        # Test session invalidation
        new_session = security_manager.create_session(user_id)
        security_manager.invalidate_session(new_session)
        assert not security_manager.validate_session(new_session), "Invalidated session accepted"
    ...
    def test_rate_limiting(self, security_manager):
        ...
        for i in range(10):
            assert security_manager.check_rate_limit(client_id), f"Normal request {i} rejected"
        ...
        assert not security_manager.check_rate_limit(client_id), "Rate limiting not working"
...
class TestAuthorization:
    ...
    def test_role_based_access_control(self, security_manager):
        ...
        for role, permissions in roles.items():
            ...
            for permission in permissions:
                assert security_manager.check_permission(user_token, permission), \
                    f"Role {role} should have permission {permission}"

            # Test unauthorized permissions
            unauthorized_perms = set(['read', 'write', 'delete', 'execute_trades', 'manage_strategies']) - set(permissions)
            for unauth_perm in unauthorized_perms:
                assert not security_manager.check_permission(user_token, unauth_perm), \
    ...
    def test_service_to_service_authorization(self, security_manager):
        ...
        for source_service, target_service in valid_communications:
            assert security_manager.authorize_service_communication(source_service, target_service), \
        ...
        for source_service, target_service in invalid_communications:
            assert not security_manager.authorize_service_communication(source_service, target_service), \
    ...
    def test_resource_access_control(self, security_manager):
        ...
        for resource, action, should_allow in resources:
            access_allowed = security_manager.check_resource_access(limited_user, resource, action)
            if should_allow:
                assert access_allowed, f"Access to {resource} ({action}) should be allowed for analyst"
            else:
                assert not access_allowed, f"Access to {resource} ({action}) should be denied for analyst"
...
class TestDataProtection:
    ...
    def test_password_hashing(self, security_manager):
        ...
        assert security_manager.verify_password(password, password_hash), "Password verification failed"

        # Test wrong password
        wrong_password = "wrong_password"
        assert not security_manager.verify_password(wrong_password, password_hash), "Wrong password accepted"
    ...
    def test_data_sanitization(self, security_manager):
        ...
        for payload in sql_payloads:
            ...
            assert "DROP" not in sanitized.upper(), f"SQL injection not sanitized: {payload}"
            assert "UNION" not in sanitized.upper(), f"SQL injection not sanitized: {payload}"
        ...
        for payload in xss_payloads:
            ...
            assert "javascript:" not in sanitized.lower(), f"XSS not sanitized: {payload}"
    ...
    def test_pii_data_masking(self, security_manager):
        ...
        for data_type, value in pii_data.items():
            ...
            assert '*' in masked_value or 'X' in masked_value, f"No masking characters in: {data_type}"
...
class TestInputValidation:
    ...
    def test_trading_parameter_validation(self, security_manager):
        ...
        assert security_manager.validate_trading_params(valid_params), "Valid trading params rejected"
        ...
        for invalid_param in invalid_params:
            ...
            assert not security_manager.validate_trading_params(combined_params), \
    ...
    def test_file_upload_security(self, security_manager):
        ...
        for filename, content in valid_files:
            assert security_manager.validate_file_upload(filename, content), \
        ...
        for filename, content in invalid_files:
            assert not security_manager.validate_file_upload(filename, content), \
...
class TestNetworkSecurity:
    ...
    def test_ssl_tls_enforcement(self, security_manager):
        ...
        for url in insecure_urls:
            assert not security_manager.is_secure_url(url), f"Insecure URL accepted: {url}"
    ...
    def test_request_header_validation(self, security_manager):
        ...
        assert security_manager.validate_request_headers(valid_headers), "Valid headers rejected"
        ...
        for malicious_header in malicious_headers:
            ...
            assert not security_manager.validate_request_headers(combined_headers), \
    ...
    def test_cors_configuration(self, security_manager):
        ...
        for origin in allowed_origins:
            assert security_manager.is_origin_allowed(origin), f"Allowed origin rejected: {origin}"
        ...
        for origin in disallowed_origins:
            assert not security_manager.is_origin_allowed(origin), f"Disallowed origin accepted: {origin}"
...
class TestDataLeakagePrevention:
    ...
    def test_log_sanitization(self, security_manager):
        ...
        for data in sensitive_data:
            ...
            assert 'password=' not in sanitized_log.lower(), "Password not masked in logs"
            assert 'api_key=' not in sanitized_log.lower(), "API key not masked in logs"
            assert '4111111111111111' not in sanitized_log, "Credit card not masked in logs"
    ...
    def test_error_message_sanitization(self, security_manager):
        ...
        for error_message in error_scenarios:
            ...
            assert 'password@' not in sanitized_error, "Database password leaked in error"
            ...
            assert 'redis_secret' not in sanitized_error, "Redis password leaked in error"
    ...
    def test_response_data_filtering(self, security_manager):
        ...
        assert 'internal_system_info' not in filtered_response, "Internal info not filtered"

        # Verify legitimate data is preserved
        assert filtered_response['user_id'] == 'user123', "User ID filtered incorrectly"
        assert filtered_response['account_balance'] == 50000.0, "Account balance filtered incorrectly"

        # Check nested filtering
        positions = filtered_response.get('positions', [])
        if positions:
            assert 'broker_account_id' not in positions[0], "Broker account ID not filtered"
...
class TestVulnerabilityPrevention:
    ...
    def test_sql_injection_prevention(self, security_manager):
        ...
        for injection_attempt in sql_injection_attempts:
            ...
            assert injection_attempt in safe_query, "Input parameter missing from safe query"
    ...
    def test_xss_prevention(self, security_manager):
        ...
        for xss_attempt in xss_attempts:
            ...
            assert "<script>" not in sanitized_output.lower(), f"Script tag not escaped: {xss_attempt}"
            assert "javascript:" not in sanitized_output.lower(), f"JavaScript protocol not escaped: {xss_attempt}"
            assert "onerror=" not in sanitized_output.lower(), f"Event handler not escaped: {xss_attempt}"
    ...
    def test_path_traversal_prevention(self, security_manager):
        ...
        for path_attempt in path_traversal_attempts:
            ...
            assert '../' not in safe_path, f"Path traversal not prevented: {path_attempt}"
            assert '..\\' not in safe_path, f"Windows path traversal not prevented: {path_attempt}"
            assert not safe_path.startswith('/'), f"Absolute path not prevented: {path_attempt}"
        ...
        for injection_attempt in command_injection_attempts:
            ...
            for char in dangerous_chars:
                if char in injection_attempt and injection_attempt != 'normal_input':
                    assert char not in sanitized_input, f"Dangerous character not removed: {char}"
...
class TestCryptographicSecurity:
    ...
    def test_hash_function_security(self, encryption_manager):
        ...
        for algorithm in hash_algorithms:
            ...
            assert hash_value != different_hash, f"Hash collision detected for {algorithm}"
    ...
    def test_digital_signature_verification(self, encryption_manager):
        ...
        is_tampered_valid = encryption_manager.verify_signature(tampered_message, signature, public_key)
        ...
        is_wrong_valid = encryption_manager.verify_signature(message, wrong_signature, public_key)
...
class TestAuditingAndCompliance:
    ...
    def test_audit_trail_creation(self, security_manager):
        ...
        operations = [
            ...
            },
            {
                ...
                'user_id': 'admin_456',
                'details': {'strategy_id': 'momentum_v1', 'changes': ['risk_limit_updated']},
                'timestamp': datetime.now(timezone.utc)
            },
            {
        ...
        for i, entry in enumerate(audit_entries):
            if i > 0:
                ...
                assert 'previous_hash' in entry, "Previous hash missing from audit chain"
    ...
    def test_compliance_data_retention(self, security_manager):
        ...
        for data_type, retention_days in data_types:
            ...
            recent_date = datetime.now(timezone.utc) - timedelta(days=retention_days // 2)
    ...
    def test_data_anonymization(self, security_manager):
        ...
        assert len(anonymized_data['trading_history']) == len(user_data['trading_history']), \
            "Trading history structure changed"

        # Verify trading data values are preserved (for analysis)
        for orig, anon in zip(user_data['trading_history'], anonymized_data['trading_history']):
            assert orig['symbol'] == anon['symbol'], "Trading symbol changed during anonymization"
...
class TestSecurityMonitoring:
    ...
    def test_anomaly_detection(self, security_manager):
        ...
        assert anomalies_detected >= 2, f"Anomaly detection too lenient: {anomalies_detected}/4 detected"
    ...
    def test_brute_force_detection(self, security_manager):
        ...
        assert failed_attempts <= 5, f"Brute force detection too lenient: {failed_attempts} attempts allowed"
        ...
        with patch('time.time', return_value=time.time() + 3600):  # 1 hour later
            unblocked_result = security_manager.record_login_attempt(client_ip, success=True)
    ...
    def test_suspicious_trading_pattern_detection(self, security_manager):
        ...
        suspicious_trades = [
            ...
            {'symbol': 'AAPL', 'quantity': 1000000, 'price': 150.0},  # Unusually large trade
        ...
        assert suspicious_count >= 2, f"Suspicious pattern detection too lenient: {suspicious_count}/3"
    ...
    def test_data_access_monitoring(self, security_manager):
        ...
        assert suspicious_access_count >= 2, f"Data access monitoring too lenient: {suspicious_access_count}/3"
...
class TestSecurityConfiguration:
    ...
    def test_environment_security_settings(self, security_manager):
        ...
        for env in environments:
            ...
            if env == 'production':
                ...
                assert env_config['debug_mode'] is False, "Debug mode enabled in production"
                assert env_config['log_level'] != 'DEBUG', "Debug logging enabled in production"
                assert env_config['require_api_key'] is True, "API key not required in production"

            elif env == 'development':
                # Development can have relaxed settings
                assert env_config['debug_mode'] is True, "Debug mode should be enabled in development"
    ...
    def test_cors_security_configuration(self, security_manager):
        ...
        for method in dangerous_methods:
            assert method not in allowed_methods, f"Dangerous HTTP method allowed: {method}"
...
class TestVulnerabilityScanning:
    ...
    def test_directory_traversal_protection(self, security_manager):
        ...
        for file_request in file_requests:
            ...
            if any(pattern in file_request for pattern in ['..', '/etc/', 'C:\\', 'system32']):
    ...
    def test_timing_attack_prevention(self, security_manager):
        ...
        max_time_variance = max(wrong_times + [correct_time]) - min(wrong_times + [correct_time])
        assert max_time_variance < 0.01, f"Timing variance too high: {max_time_variance:.6f}s"
...
class TestSecretManagement:
    ...
    def test_environment_variable_security(self, security_manager):
        ...
        for env_var in sensitive_env_vars:
            ...
            assert log_safe_value != test_value, f"Environment variable not masked: {env_var}"
            assert '*' in log_safe_value or 'X' in log_safe_value, f"No masking applied to: {env_var}"
    ...
    def test_secure_configuration_storage(self, security_manager):
        ...
        assert raw_stored_config != json.dumps(sensitive_config), "Configuration not encrypted at rest"
...
class TestNetworkSecurityTests:
    ...
    def test_request_size_limits(self, security_manager):
        ...
        assert security_manager.validate_request_size(json.dumps(normal_request)), "Normal request rejected"

        # Large request
        large_request = {'data': 'A' * (10 * 1024 * 1024)}  # 10MB
        assert not security_manager.validate_request_size(json.dumps(large_request)), "Large request accepted"
    ...
    def test_user_agent_validation(self, security_manager):
        ...
        for user_agent in valid_user_agents:
            assert security_manager.validate_user_agent(user_agent), f"Valid user agent rejected: {user_agent}"
        ...
        for user_agent in suspicious_user_agents:
            assert not security_manager.validate_user_agent(user_agent), f"Suspicious user agent accepted: {user_agent}"
...
class TestSecurityStressTesting:
    ...
    async def test_concurrent_authentication_stress(self, security_manager):
        ...
        success_rate = total_successes / (concurrent_users * (attempts_per_user // 3))  # Only valid attempts

        # Verify system handled load
        assert success_rate > 0.9, f"Authentication success rate too low under load: {success_rate*100:.1f}%"
        assert total_attempts > 0, f"No authentication attempts made: {total_attempts}"
        assert duration < 60, f"Authentication stress test took too long: {duration:.2f}s"

        print(f"Authentication stress test: {success_rate*100:.1f}% success rate in {duration:.2f}s")
    ...
    async def test_rate_limiting_under_load(self, security_manager):
        ...
        assert block_rate > 0.5, f"Rate limiting not effective: {block_rate*100:.1f}% blocked"

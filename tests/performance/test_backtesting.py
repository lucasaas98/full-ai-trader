...
class MonteCarloSimulator:
    ...
    def run_simulation(
        self, num_simulations: int = 1000, time_horizon: int = 252
    ) -> MonteCarloResult:
        ...
        return MonteCarloResult(
            ...
            probability_of_loss=float(
                np.sum(final_values < self.initial_capital) / num_simulations
            ),
        )
...
class TestBacktestingPerformance:
    ...
    def test_large_dataset_backtesting(self):
        ...
        dates_list = [
            datetime.fromtimestamp(ts.timestamp(), tz=timezone.utc) for ts in dates
        ]
        np.random.seed(42)
    ...
    def test_multiple_symbol_backtesting(self, sample_price_data):
        ...
        for i, symbol in enumerate(symbols):
            for j in range(20):
                signal = TradeSignal(
                    ...
                    stop_loss=Decimal(
                        str(sample_price_data[symbol].iloc[j * 10] * 0.95)
                    ),
                    take_profit=Decimal(
                        str(sample_price_data[symbol].iloc[j * 10] * 1.05)
                    ),
                )
                signals.append(signal)
        ...
        assert result.total_trades > 0
        assert (
            len(symbols) <= result.total_trades
        ...
        def run_single_backtest(seed: int) -> PerformanceMetrics:
            ...
            for i in range(50):
                signal = TradeSignal(
                    symbol="AAPL",
                    signal_type=(
                        SignalType.BUY if np.random.random() > 0.5 else SignalType.SELL
                    ),
                    confidence=0.7,
                    price=Decimal(str(prices[i])),
                    quantity=100,
        ...
        for data_size in [100, 500, 1000, 2000]:
            ...
            for i in range(num_signals):
                signal = TradeSignal(
                    ...
                    stop_loss=Decimal(
                        str(prices[i * (data_size // num_signals)] * 0.95)
                    ),
                    take_profit=Decimal(
                        str(prices[i * (data_size // num_signals)] * 1.05)
                    ),
                )
                signals.append(signal)
    ...
    def test_high_frequency_signal_processing(self):
        ...
        prices_series = 100 * np.cumprod(
            1 + np.random.normal(0, 0.0005, len(timestamps))
        )

...
class BacktestEngine:
    ...
    def run_backtest(self, strategy=None, symbols=None, data_source=None,
                    strategy_func=None, data=None, initial_capital=None,
...
class TradingSignal:
    def __init__(self, symbol, signal_type=None, price=None, timestamp=None,
                 action=None, quantity=None, confidence=None, strategy=None,
...
class BacktestDataGenerator:
    ...
    def generate_multi_asset_data(
        ...
        for i, symbol in enumerate(symbols):
            ...
            returns = random_matrix[:, i] * 0.02 + 0.0001  # 2% volatility, slight positive drift
...
class TestBacktestEngine:
    ...
    def test_simple_buy_and_hold_strategy(self, backtest_engine, sample_historical_data):
        ...
        assert result.execution_time_ms < 5000, f"Backtest too slow: {execution_time:.2f}ms"
        ...
        print(f"Buy and hold backtest: {result.total_return*100:.2f}% return, {result.total_trades} trades")
    ...
    def test_momentum_strategy(self, backtest_engine, sample_historical_data):
        ...
        assert result.total_trades >= 2, f"Too few trades for momentum strategy: {result.total_trades}"
        assert abs(result.total_return) < 2.0, f"Unrealistic return: {result.total_return*100:.2f}%"
        assert result.max_drawdown >= 0, "Drawdown should be non-negative"

        print(f"Momentum strategy: {result.total_return*100:.2f}% return, "
    ...
    def test_mean_reversion_strategy(self, backtest_engine, sample_historical_data):
        ...
        assert -1.0 <= result.total_return <= 1.0, f"Unrealistic return: {result.total_return*100:.2f}%"

        print(f"Mean reversion strategy: {result.total_return*100:.2f}% return, "
    ...
    def test_multi_asset_portfolio_strategy(self, backtest_engine, multi_asset_data):
        ...
        def portfolio_rebalancing_strategy(all_data: Dict[str, pd.DataFrame]) -> List[TradingSignal]:
        ...
        assert result.total_trades >= len(multi_asset_data), "Insufficient trades for portfolio"
        assert result.final_value > 0, "Portfolio value is zero"

        print(f"Portfolio strategy: {result.total_return*100:.2f}% return across {len(multi_asset_data)} assets")

    def test_backtest_performance_metrics(self, backtest_engine, sample_historical_data):
        ...
        assert 0 <= result.max_drawdown <= 1, f"Invalid max drawdown: {result.max_drawdown}"
        ...
        print(f"Performance metrics test: Sharpe={result.sharpe_ratio:.2f}, "
...
class TestMonteCarloSimulation:
    ...
    def test_basic_monte_carlo_simulation(self, monte_carlo_simulator, sample_historical_data):
        ...
        assert len(simulation_results) == 100, f"Expected 100 simulations, got {len(simulation_results)}"
        ...
        assert -0.5 < mean_return < 0.5, f"Unrealistic mean return: {mean_return*100:.2f}%"
        assert 0 < std_return < 0.3, f"Unrealistic return volatility: {std_return*100:.2f}%"

        print(f"Monte Carlo (100 runs): Mean return={mean_return*100:.2f}%, "
    ...
    def test_portfolio_monte_carlo(self, monte_carlo_simulator, multi_asset_data):
        ...
        def balanced_portfolio_strategy(all_data: Dict[str, pd.DataFrame]) -> List[TradingSignal]:
            """Balanced portfolio strategy."""
            signals = []

            for symbol, data in all_data.items():
                if len(data) % 200 == 0 and len(data) > 0:  # Rebalance every 200 periods
                    current = data.iloc[-1]
        ...
        assert len(portfolio_results) == 50, f"Expected 50 portfolio simulations, got {len(portfolio_results)}"

        # Analyze portfolio performance
        portfolio_returns = [sim.total_return for sim in portfolio_results]
        portfolio_sharpes = [sim.sharpe_ratio for sim in portfolio_results if sim.sharpe_ratio is not None]
        ...
        assert -0.3 < mean_portfolio_return < 0.3, f"Unrealistic portfolio return: {mean_portfolio_return*100:.2f}%"

        print(f"Portfolio Monte Carlo: Mean return={mean_portfolio_return*100:.2f}%, "
              f"Mean Sharpe={mean_sharpe:.2f}")

    def test_risk_analysis_simulation(self, monte_carlo_simulator, sample_historical_data):
        ...
        assert 'max_drawdown_distribution' in risk_results, "Max drawdown distribution not calculated"
    ...
    def test_walk_forward_analysis(self, backtest_engine, sample_historical_data):
        ...
        def adaptive_strategy(data: pd.DataFrame, lookback_period: int = 50) -> List[TradingSignal]:
        ...
        assert len(walk_forward_results) >= 3, f"Expected multiple walk-forward periods, got {len(walk_forward_results)}"
        ...
        assert return_std < 0.2, f"Strategy too unstable across periods: {return_std*100:.2f}% std"

        print(f"Walk-forward analysis: {len(walk_forward_results)} periods, "
...
class TestBacktestingInfrastructure:
    ...
    def test_historical_data_validation(self, sample_historical_data):
        ...
        for col in required_columns:
            assert col in sample_historical_data.columns, f"Missing required column: {col}"

        # Verify data quality
        assert len(sample_historical_data) > 1000, f"Insufficient data points: {len(sample_historical_data)}"
        assert sample_historical_data['price'].isna().sum() == 0, "Price data contains NaN values"
        assert (sample_historical_data['price'] > 0).all(), "Price data contains non-positive values"
        assert (sample_historical_data['volume'] >= 0).all(), "Volume data contains negative values"

        # Verify timestamp ordering
        timestamps = pd.to_datetime(sample_historical_data['timestamp'])
        assert timestamps.is_monotonic_increasing, "Timestamps are not in chronological order"
        ...
        assert (spreads < sample_historical_data['price'] * 0.01).all(), "Unrealistic bid/ask spreads"

        print(f"Data validation passed: {len(sample_historical_data)} data points, "
              f"{sample_historical_data['timestamp'].min()} to {sample_historical_data['timestamp'].max()}")

    def test_backtest_parameter_optimization(self, backtest_engine, sample_historical_data):
        """Test parameter optimization functionality."""

        def parameterized_strategy(data: pd.DataFrame, ma_short: int = 10, ma_long: int = 20) -> List[TradingSignal]:
            ...
            if previous is not None:
                # Buy signal: short MA crosses above long MA
                if (current[f'sma_{ma_short}'] > current[f'sma_{ma_long}'] and
                    previous[f'sma_{ma_short}'] <= previous[f'sma_{ma_long}']):
        ...
        best_result = max(optimization_results, key=lambda x: x['sharpe'] if x['sharpe'] else -999)
        ...
        print(f"Parameter optimization: Best params {best_result['parameters']} "
    ...
    def test_backtest_data_pipeline(self, backtest_engine):
        ...
        assert return_std < 0.1, f"Results too different across sources: {return_std*100:.2f}% std"

        print(f"Multi-source backtest: {return_std*100:.2f}% return variance across sources")
...
class TestAdvancedBacktesting:
    ...
    def test_multi_timeframe_strategy(self, backtest_engine, sample_historical_data):
        ...
        def multi_timeframe_strategy(data: pd.DataFrame) -> List[TradingSignal]:
            ...
            if len(data_5m) >= 20 and len(data_15m) >= 10:
                ...
                short_trend = (data_5m.iloc[-1]['price'] - data_5m.iloc[-10]['price']) / data_5m.iloc[-10]['price']

                # Medium-term trend (15-minute)
                medium_trend = (data_15m.iloc[-1]['price'] - data_15m.iloc[-5]['price']) / data_15m.iloc[-5]['price']
        ...
        print(f"Multi-timeframe strategy: {result.total_return*100:.2f}% return, "
              f"{result.total_trades} trades")

    def test_strategy_comparison_framework(self, backtest_engine, sample_historical_data):
        ...
        strategies = {
            ...
            ] if len(data) % 50 == 0 and len(data) > 0 else [],
            ...
            ] if len(data) % 60 == 0 and len(data) > 0 else []
        }
        ...
        # Compare strategies
        best_strategy = max(strategy_results.items(), key=lambda x: x[1].sharpe_ratio or -999)
        worst_strategy = min(strategy_results.items(), key=lambda x: x[1].sharpe_ratio or 999)

        print("Strategy comparison:")
        for name, result in strategy_results.items():
            print(f"  {name}: {result.total_return*100:.2f}% return, "
        ...
        assert all(result.total_trades >= 0 for result in strategy_results.values()), "Negative trade counts"
    ...
    def test_custom_metrics_calculation(self, backtest_engine, sample_historical_data):
        ...
        if result.calmar_ratio is not None:
            assert -10 < result.calmar_ratio < 10, f"Unrealistic Calmar ratio: {result.calmar_ratio}"
...
def test_backtesting_performance_benchmark():
    ...
    assert execution_time < 60000, f"Backtest too slow: {execution_time:.2f}ms for {len(large_dataset)} points"
    assert data_points_per_ms > 10, f"Processing rate too low: {data_points_per_ms:.2f} points/ms"

    print(f"Backtesting benchmark: {execution_time:.2f}ms for {len(large_dataset)} data points "

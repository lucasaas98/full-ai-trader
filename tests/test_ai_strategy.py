...
from services.strategy_engine.src.ai_models import (
    AIDecisionRecord, AIPerformanceMetrics, create_performance_summary
)
...
class TestDataContextBuilder:
    ...
    def test_rsi_calculation(self, sample_price_data):
        ...
        # Test edge cases
        flat_data = sample_price_data.with_columns(
...
class TestAIStrategyEngine:
    ...
    def sample_data(self):
        ...
        return pl.DataFrame({
            'timestamp': [datetime.now() - timedelta(hours=i) for i in range(100, 0, -1)],
    ...
    async def test_analyze(self, mock_client_class, strategy_config, sample_data):
        ...
        with patch('builtins.open', mock_open(read_data=yaml.dump({'prompts': {}}))):
            ...
            strategy.query_multiple_models = AsyncMock(return_value=[mock_client.query.return_value])
    ...
    async def test_position_limits(self, mock_redis, integration_config):
        ...
        with patch.dict(os.environ, {'ANTHROPIC_API_KEY': 'test_key'}):
            with patch('services.strategy_engine.src.ai_integration.create_async_engine'):
                ...
                signal = Signal(action=SignalType.BUY, confidence=80, position_size=0.05)
        ...
        with patch.dict(os.environ, {'ANTHROPIC_API_KEY': 'test_key'}):
            with patch('services.strategy_engine.src.ai_integration.create_async_engine'):
                ...
                await integration._publish_exit_signal(
                    'TEST',
                    integration.active_positions['TEST'],
                    'stop_loss'
                )
...
class TestPerformanceTracking:
    ...
    def test_performance_summary_creation(self):
        ...
        executions = [
            MagicMock(realized_pnl=100.0),
            MagicMock(realized_pnl=-50.0)
        ]

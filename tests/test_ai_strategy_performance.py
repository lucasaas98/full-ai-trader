        ...
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test_key"}):
            with patch(
                ...
                integration = AIStrategyIntegration(
                    mock_redis,
                    "postgresql://test",
                    {"min_confidence": 60, "max_positions": 5},
                )
                ...
                with patch.object(
                    integration.ai_strategy, "analyze", return_value=mock_signal
                ):
        ...
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test_key"}):
            with patch(
                ...
                for ticker in tickers:
                    ...
                    with patch.object(
                        integration.ai_strategy, "analyze"
                    ) as mock_analyze:
                        mock_analyze.return_value = Signal(
                            action=SignalType.BUY, confidence=70, position_size=0.05
                        )
...
class TestCostOptimization:
    ...
    async def test_cache_hit_rate(self):
        ...
        prompts = [f"Analyze TEST with price {100 + i * 0.01}" for i in range(10)]
    ...
    async def test_throughput_under_load(self):
        ...
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test_key"}):
            with patch(
                ...
                start = time.time()
                tasks = [
                    integration._analyze_ticker(f"TEST{i}") for i in range(num_tickers)
                ]
                await asyncio.gather(*tasks)
                elapsed = time.time() - start

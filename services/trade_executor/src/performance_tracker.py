...
class PerformanceTracker:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self._redis = redis.from_url(
                self.config.redis.url, max_connections=5, retry_on_timeout=True
            )
    ...
    async def calculate_daily_performance(
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                win_rate = Decimal(daily_stats["winning_trades"]) / Decimal(
                ...
                avg_win = (
                    gross_profit / Decimal(daily_stats["winning_trades"])
                    if daily_stats["winning_trades"] > 0
                    else Decimal("0")
                )
                avg_loss = (
                    gross_loss / Decimal(daily_stats["losing_trades"])
                    if daily_stats["losing_trades"] > 0
                    else Decimal("0")
                )
                expectancy = (win_rate * avg_win) - (
                    (Decimal("1") - win_rate) * avg_loss
                )
                ...
                if daily_stats["pnl_stddev"] and daily_stats["pnl_stddev"] > 0:
                    risk_free_rate = Decimal("0.02") / Decimal(
                        "365"
                    )  # Assume 2% annual risk-free rate
                    excess_return = (
                        Decimal(str(daily_stats["avg_pnl"])) - risk_free_rate
                    )
                    calculated_sharpe = excess_return / Decimal(
                        str(daily_stats["pnl_stddev"])
                    )
                    # Clamp the value to database field limits (DECIMAL(8,6) = ±99.999999)
                    sharpe_ratio = max(
                        Decimal("-99.999999"),
                        min(Decimal("99.999999"), calculated_sharpe),
                    )
                ...
                metrics = {
                    ...
                    "return_pct": (
                        float(
                            Decimal(str(daily_stats["net_pnl"]))
            ...
            logger.error(
                f"Failed to calculate daily performance for {target_date}: {e}"
            )
            return self._empty_daily_metrics(
                target_date if target_date else date.today()
            )
    ...
    async def _get_portfolio_value_for_date(self, target_date: date) -> Decimal:
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                return (
                    Decimal(str(total_value)) if total_value else Decimal("100000")
                )  # Default value
    ...
    async def calculate_strategy_performance(
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                # Calculate expectancy
                avg_win = (
                    gross_profit / strategy_stats["winning_trades"]
                    if strategy_stats["winning_trades"] > 0
                    else Decimal("0")
                )
                avg_loss = (
                    gross_loss / strategy_stats["losing_trades"]
                    if strategy_stats["losing_trades"] > 0
                    else Decimal("0")
                )
                ...
                if strategy_stats["pnl_stddev"] and strategy_stats["pnl_stddev"] > 0:
                    ...
                    sharpe_ratio = (
                        excess_return / strategy_stats["pnl_stddev"] * math.sqrt(252)
                    )  # Annualized

                # Get consecutive wins/losses
                win_loss_streak = await self._calculate_win_loss_streaks(
                    strategy_name, days
                )
                ...
                return {
                    ...
                    "avg_holding_hours": float(
                        strategy_stats["avg_holding_hours"] or 0
                    ),
            ...
            logger.error(
                f"Failed to calculate strategy performance for {strategy_name}: {e}"
            )
            return {}

    async def _calculate_win_loss_streaks(
        self, strategy_name: str, days: int
    ) -> Dict[str, Any]:
    ...
    async def calculate_risk_metrics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                sharpe_ratio = (
                    (excess_return / return_std * math.sqrt(252))
                    if return_std > 0
                    else 0
                )
                ...
                sortino_ratio = (
                    (excess_return / downside_std * math.sqrt(252))
                    if downside_std > 0
                    else 0
                )
                ...
                return {
                    ...
                    "total_return": (
                        cumulative_returns[-1] - 1 if cumulative_returns else 0
                    ),
                    "calmar_ratio": (
                        (cumulative_returns[-1] - 1) / max_drawdown
                        if max_drawdown > 0
                        else 0
                    ),
                }
    ...
    async def calculate_monthly_performance(
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                win_rate = (
                    monthly_stats["winning_trades"] / monthly_stats["total_trades"]
                )
                ...
                if daily_data:
                    ...
                    avg_portfolio_value = sum(
                        float(d["portfolio_value"]) for d in daily_data
                    ) / len(daily_data)
                    monthly_return_pct = (
                        (monthly_return / avg_portfolio_value * 100)
            ...
            logger.error(
                f"Failed to calculate monthly performance for {year}-{month}: {e}"
            )
            return {}
            ...
            async with self._db_pool.acquire() as conn:
                ...
                for row in rows:
                    ...
                    trade_data = {
                        ...
                        # Additional metrics
                        "return_pct": (
                            float(row["return_percentage"])
                            if row["return_percentage"]
                            else None
                        ),
                        "duration_hours": (
                            float(row["holding_period_hours"])
                            if row["holding_period_hours"]
                            else None
                        ),
                        "mfe": (
                            float(row["max_favorable_excursion"])
                            if row["max_favorable_excursion"]
                            else None
                        ),
                        "mae": (
                            float(row["max_adverse_excursion"])
                            if row["max_adverse_excursion"]
                            else None
                        ),
                        # Stop loss and take profit
                        "stop_loss": (
                            float(row["stop_loss"]) if row["stop_loss"] else None
                        ),
                        "take_profit": (
                            float(row["take_profit"]) if row["take_profit"] else None
                        ),
                        # Notes
        ...
        try:
            ...
            for strategy in strategies:
                ...
                strategy_performance[strategy_name] = (
                    await self.calculate_strategy_performance(strategy_name, days)
                )

            # Get monthly performance
            current_date = date.today()
            monthly_perf = await self.calculate_monthly_performance(
            ...
            async with self._db_pool.acquire() as conn:
                ...
                for row in rows:
                    trade_data = {
                        ...
                        "strategy": row["strategy_type"],
                        "duration_hours": (
                            (row["exit_time"] - row["entry_time"]).total_seconds()
                            / 3600
                            if row["exit_time"]
                            else None
                        ),
    ...
    async def calculate_trade_quality_score(self, position_id: UUID) -> float:
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                if trade_data["slippage"]:
                    slippage_penalty = min(
                        float(trade_data["slippage"]) * 1000, 20
                ...
                if trade_data["pnl"] and trade_data["pnl"] > 0:
                    ...
                    if trade_data["take_profit"] and trade_data["exit_price"]:
                        target_achievement = (
                            abs(trade_data["exit_price"] - trade_data["take_profit"])
                ...
                if trade_data["max_adverse_excursion"] and trade_data["pnl"]:
                    mae_ratio = abs(
                        float(trade_data["max_adverse_excursion"])
                ...
                if trade_data["max_favorable_excursion"] and trade_data["pnl"]:
                    mfe_ratio = float(trade_data["max_favorable_excursion"]) / float(
                        trade_data["pnl"]
                    )
            ...
            logger.error(
                f"Failed to calculate trade quality score for {position_id}: {e}"
            )
            return 0.0
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                return {
                    ...
                    "total_market_value": (
                        float(metrics["total_market_value"])
                        if metrics and metrics["total_market_value"]
                        else 0
                    ),
                    "unrealized_pnl": (
                        float(metrics["total_unrealized_pnl"])
                        if metrics and metrics["total_unrealized_pnl"]
                        else 0
                    ),
                    "total_equity": (
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                if deleted_count and deleted_count > 0:
                    logger.info(
                        f"Cleaned up {deleted_count} old trade performance records"
                    )
    ...
    async def export_csv_report(
        ...
        """
        try:
            trades_data = await self.export_for_tradenote(
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                for row in monthly_data:
                    month_return_pct = (
                        (
                            float(row["monthly_pnl"])
                            / float(row["avg_portfolio_value"])
                            * 100
                        )
                        if row["avg_portfolio_value"] > 0
                        else 0
                    )
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                return {
                    ...
                    "best_strategy": (
                        strategy_comparison[0] if strategy_comparison else None
                    ),
            ...
            for i, symbol1 in enumerate(symbols):
                ...
                for j, symbol2 in enumerate(symbols):
                    if i == j:
                        ...
                        correlation_matrix[symbol1][symbol2] = correlation_matrix[
                            symbol2
                        ][symbol1]
                    else:
                        # Calculate correlation
                        corr = await self._calculate_symbol_correlation(
    ...
    async def _calculate_symbol_correlation(
        self, symbol1: str, symbol2: str, days: int
    ) -> float:

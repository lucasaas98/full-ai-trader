...
class BaseStrategy(ABC):
    ...
    def get_exit_signal(self, symbol: str, data: pl.DataFrame,
        ...
        try:
            ...
            if current_position:
                exit_signal = self._check_exit_conditions(signal, current_position, data)
                if exit_signal:
                    return exit_signal
    ...
    def _check_exit_conditions(self, signal: Signal, position: Dict,
                              data: pl.DataFrame) -> Optional[Signal]:
    ...
    def _enhance_signal(self, symbol: str, signal: Signal, data: pl.DataFrame) -> Signal:
    ...
    async def backtest(self, symbol: str, data: pl.DataFrame,
                      start_date: datetime, end_date: datetime,
                      initial_capital: float = 100000.0) -> BacktestMetrics:
        """
        ...
        for i in range(window_size, test_data.height):
            ...
            try:
                ...
                if position is None and signal.action in [SignalType.BUY, SignalType.SELL]:
                    ...
                    if (signal.action == SignalType.CLOSE or
                        (signal.action == SignalType.SELL and position["side"] == "long") or
                        (signal.action == SignalType.BUY and position["side"] == "short")):
                        should_exit = True
                    ...
                    if should_exit:
                        ...
                        trades.append({
                            ...
                            "duration_days": (current_time - position["entry_time"]).days
                        })
    ...
    def _calculate_backtest_metrics(self, trades: List[Dict], equity_curve: List[Dict],
                                   initial_capital: float, final_capital: float) -> BacktestMetrics:
        ...
        downside_deviation = negative_returns.std() * (252 ** 0.5) if len(negative_returns) > 1 else 0
    ...
    def validate_data(self, data: pl.DataFrame) -> bool:
        ...
        if data.height < self.config.lookback_period:
            self.logger.error(f"Insufficient data: {data.height} rows, "
    ...
    def load_state(self, state: Dict[str, Any]) -> None:
        ...
        if "config" in state:
            ...
            self.config = StrategyConfig(
                ...
                default_take_profit_pct=config_data.get("default_take_profit_pct", 0.04),

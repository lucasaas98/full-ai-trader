...
class TestDataGenerator:
    ...
    def generate_realistic_price_series(self,
                                      ...
                                      include_splits: bool = False) -> pd.DataFrame:
    ...
    def generate_intraday_data(self,
                             ...
                             start_price: float = 100.0,
                             volatility: float = 0.2,
                             num_minutes: int = 390) -> pd.DataFrame:
    ...
    def generate_multi_asset_data(self,
        ...
        mean_returns = np.random.normal(0.0005, 0.0002, len(symbols))  # Different expected returns
        volatilities = np.random.normal(0.02, 0.005, len(symbols))    # Different volatilities
    ...
    def generate_market_regime_data(self,
        ...
        # Create OHLCV data
        ohlcv_data = []
        for i, (date, close_price, volume) in enumerate(zip(dates, prices[1:], volumes)):
    ...
    def generate_earnings_event_data(self,
                                   symbol: str,
                                   base_data: pd.DataFrame,
                                   earnings_dates: List[datetime]) -> pd.DataFrame:
        ...
        for earnings_date in earnings_dates:
            ...
            try:
                earnings_idx = _safe_index_to_int(data[data['timestamp'].dt.date == earnings_date.date()].index[0])
            except IndexError:
                continue
            ...
            if earnings_idx < len(data) - 1:
                ...
                data.loc[max(0, earnings_idx-1):earnings_idx+1, 'volume'] *= volume_spike

        return data

    def generate_market_crash_scenario(self,
                                     base_data: pd.DataFrame,
                                     crash_date: datetime,
                                     crash_severity: float = 0.20) -> pd.DataFrame:
        ...
        try:
            crash_idx = _safe_index_to_int(data[data['timestamp'].dt.date >= crash_date.date()].index[0])
        except IndexError:
            return data
    ...
    def generate_news_sentiment_data(self,
                                   symbol: str,
                                   date_range: Tuple[datetime, datetime],
                                   correlation_with_returns: float = 0.3) -> pd.DataFrame:
        ...
        for date in dates:
            ...
            num_articles = int(base_articles * news_multiplier * np.random.lognormal(0, 0.5))
            ...
            sentiment_data.append({
                ...
                'positive_articles': int(num_articles * max(0, sentiment_score + 1) / 2),
                'negative_articles': int(num_articles * max(0, 1 - sentiment_score) / 2),
                'neutral_articles': num_articles - int(num_articles * abs(sentiment_score)),
            })

        return pd.DataFrame(sentiment_data)

    def generate_economic_indicators_data(self,
    ...
    def apply_stress_scenario(self,
        ...
        try:
            start_idx = _safe_index_to_int(data[data['timestamp'].dt.date == start_date.date()].index[0])
        except IndexError:
            return data
    ...
    def generate_algorithmic_trading_patterns(self,
                                            base_data: pd.DataFrame,
                                            pattern_type: str = 'momentum') -> pd.DataFrame:

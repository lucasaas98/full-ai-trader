...
from .base_strategy import BaseStrategy, Signal, StrategyMode, SignalType
...
class MultiTimeFrameAnalyzer:
    ...
    def _get_timeframe_weights(self) -> Dict[str, float]:
        ...
        weight_configs = {
            StrategyMode.DAY_TRADING: {
                '1m': 0.15, '5m': 0.25, '15m': 0.30, '30m': 0.20, '1h': 0.10
            },
            StrategyMode.SWING_TRADING: {
                '15m': 0.10, '30m': 0.15, '1h': 0.25, '4h': 0.30, '1d': 0.20
            },
            StrategyMode.POSITION_TRADING: {
                '4h': 0.10, '12h': 0.15, '1d': 0.25, '1w': 0.30, '1M': 0.20
            }
        }

        return weight_configs.get(self.strategy_mode, weight_configs[StrategyMode.SWING_TRADING])
    ...
    async def confirm_signal(
        ...
        try:
            ...
            if len(available_timeframes) < self.timeframe_config['min_confirmation_count']:
                return self._create_insufficient_data_result(primary_signal, available_timeframes)

            # Generate signals for each timeframe
            for timeframe in available_timeframes:
                if timeframe in multi_timeframe_data:
                    tf_signal = await self._analyze_timeframe(
                        symbol, timeframe, multi_timeframe_data[timeframe], strategy_instance
                    )
                    if tf_signal:
    ...
    async def _analyze_timeframe(
        ...
        try:
            ...
            volumes = data.select("volume").to_numpy().flatten() if "volume" in data.columns else None

            # Calculate key indicators
            indicators = self._calculate_timeframe_indicators(close_prices, high_prices, low_prices, volumes)
            ...
            signal_type = self._determine_signal_type(indicators, trend_direction, strength)

            # Calculate confidence
            confidence = self._calculate_timeframe_confidence(indicators, trend_direction, strength)

            # Volume confirmation
            volume_confirmation = self._check_volume_confirmation(volumes, trend_direction) if volumes is not None else False
    ...
    def _calculate_timeframe_indicators(
        ...
        try:
            ...
            if len(close) >= 14:
                ...
                indicators['high_low_position'] = (current_price - recent_low) / (recent_high - recent_low)
            ...
            if volumes is not None and len(volumes) >= 10:
                ...
                longer_vol_avg = np.mean(vol[-20:]) if len(vol) >= 20 else recent_vol_avg
                indicators['volume_trend'] = (recent_vol_avg - longer_vol_avg) / longer_vol_avg if longer_vol_avg > 0 else 0
    ...
    def _determine_trend_direction(self, close_prices: Any, indicators: Dict[str, float]) -> str:
    ...
    def _calculate_signal_strength(self, indicators: Dict[str, float], trend_direction: str) -> float:
        ...
        try:
            ...
            # Momentum strength
            if 'momentum' in indicators:
                momentum_strength = min(abs(indicators['momentum']) * 10, 1.0)  # Scale to 0-1
            ...
            if 'price_vs_sma20' in indicators:
                sma_strength = min(abs(indicators['price_vs_sma20']) * 5, 1.0)  # Scale to 0-1
            ...
            if 'volume_trend' in indicators and trend_direction != 'sideways':
                ...
                if (trend_direction == 'up' and vol_trend > 0) or (trend_direction == 'down' and vol_trend > 0):
    ...
    def _determine_signal_type(self, indicators: Dict[str, float], trend_direction: str, strength: float) -> SignalType:
    ...
    def _calculate_timeframe_confidence(self, indicators: Dict[str, float], trend_direction: str, strength: float) -> float:
        ...
        try:
            ...
            if trend_direction != 'sideways':
                # Momentum boost
                if 'momentum' in indicators:
                    momentum_boost = min(abs(indicators['momentum']) * 20, 10)  # Max 10 point boost
                    base_confidence += momentum_boost

                # Volume confirmation boost
                if 'volume_trend' in indicators:
                    vol_trend = indicators['volume_trend']
                    if (trend_direction == 'up' and vol_trend > 0.1) or (trend_direction == 'down' and vol_trend > 0.1):
                        base_confidence += 5
    ...
    async def _perform_confirmation_analysis(
        ...
        try:
            ...
            weighted_buy_score = sum(s.confidence * self.timeframe_weights.get(s.timeframe, 0.2) for s in buy_signals)
            weighted_sell_score = sum(s.confidence * self.timeframe_weights.get(s.timeframe, 0.2) for s in sell_signals)
            ...
            confirmation_strength = self._determine_confirmation_strength(support_ratio, conflict_ratio)

            # Determine alignment type
            alignment_type = self._determine_alignment_type(buy_count, sell_count, hold_count, total_signals)

            # Calculate overall confidence
            base_confidence = dominant_score / len(supporting_timeframes) if supporting_timeframes else 50.0
            ...
            optimal_entry_tf = self._find_optimal_entry_timeframe(supporting_timeframes, timeframe_signals)
    ...
    def _determine_confirmation_strength(self, support_ratio: float, conflict_ratio: float) -> ConfirmationStrength:
    ...
    def _determine_alignment_type(self, buy_count: int, sell_count: int, hold_count: int, total: int) -> TimeFrameAlignment:
    ...
    def _find_optimal_entry_timeframe(self, supporting_timeframes: List[str], all_signals: List[TimeFrameSignal]) -> Optional[str]:
        ...
        supporting_signals = [s for s in all_signals if s.timeframe in supporting_timeframes]
        ...
        for signal in supporting_signals:
            ...
            score = (signal.confidence * 0.4) + (signal.strength * 100 * 0.4) + (weight * 100 * 0.2)
    ...
    def _enhance_confirmation_with_analysis(
        ...
        try:
            ...
            momentum_values = [s.momentum for s in timeframe_signals if hasattr(s, 'momentum')]
            if momentum_values:
            ...
            trend_consistency = trend_directions.count(dominant_trend) / len(trend_directions)
            ...
            if len(confirmation.conflicting_timeframes) > self.timeframe_config['max_conflicting_allowed']:
                risk_factors.append("Too many conflicting timeframes")

            # Check for low confidence signals
            low_confidence_count = sum(1 for s in timeframe_signals if s.confidence < 60)
            ...
            confluence_boost = len(confluence_factors) * 5  # 5 points per confluence factor
            risk_penalty = len(risk_factors) * 10  # 10 points per risk factor

            confirmation.overall_confidence = min(
                max(confirmation.overall_confidence + confluence_boost - risk_penalty, 0.0),
                100.0
            )
    ...
    def _create_insufficient_data_result(self, primary_signal: Signal, available_timeframes: List[str]) -> MultiTimeFrameConfirmation:
        ...
        return MultiTimeFrameConfirmation(
            ...
            risk_factors=[f"Insufficient timeframe data: only {len(available_timeframes)} available"],
            metadata={"available_timeframes": available_timeframes}
        )

    def _create_error_result(self, primary_signal: Signal, error_message: str) -> MultiTimeFrameConfirmation:
    ...
    def should_allow_entry(self, confirmation: MultiTimeFrameConfirmation) -> Tuple[bool, str]:
        ...
        try:
            ...
            current_strength_level = strength_levels.get(confirmation.confirmation_strength, 0)
            required_strength_level = strength_levels.get(required_strength, 3)

            if current_strength_level < required_strength_level:
                return False, f"Insufficient confirmation strength: {confirmation.confirmation_strength.value} (required: {required_strength.value})"
            ...
            if confirmation.overall_confidence < min_confidence:
                return False, f"Confidence too low: {confirmation.overall_confidence:.1f}% (required: {min_confidence}%)"

            # All checks passed
            return True, f"Multi-timeframe confirmation passed: {confirmation.confirmation_strength.value} with {confirmation.overall_confidence:.1f}% confidence"
...
class MultiTimeFrameSignalEnhancer:
    ...
    async def enhance_signal_with_confirmation(
        ...
        try:
            ...
            error_confirmation = self.analyzer._create_error_result(primary_signal, str(e))
    ...
    def _create_enhanced_signal(
        ...
        try:
            ...
            if allow_entry and confirmation.primary_signal != SignalType.HOLD:
                ...
                confirmation_multiplier = self._get_confirmation_multiplier(confirmation.confirmation_strength)
                adjusted_position_size = primary_signal.position_size * confirmation_multiplier
            ...
            enhanced_metadata.update({
                ...
                "original_signal": {
                    "action": primary_signal.action.value if hasattr(primary_signal.action, 'value') else str(primary_signal.action),
    ...
    def _build_enhanced_reasoning(
        ...
        try:
            ...
            reasoning_parts.append(f"Multi-Timeframe Analysis: {confirmation.confirmation_strength.value}")
            ...
            if confirmation.supporting_timeframes:
                reasoning_parts.append(f"Supporting timeframes: {', '.join(confirmation.supporting_timeframes)}")

            if confirmation.conflicting_timeframes:
                reasoning_parts.append(f"Conflicting timeframes: {', '.join(confirmation.conflicting_timeframes)}")

            # Confluence factors
            if confirmation.key_confluence_factors:
                reasoning_parts.append(f"Confluence factors: {'; '.join(confirmation.key_confluence_factors)}")

            # Risk factors
            if confirmation.risk_factors:
                reasoning_parts.append(f"Risk factors: {'; '.join(confirmation.risk_factors)}")
...
def create_multi_timeframe_analyzer(strategy_mode: StrategyMode) -> Tuple[MultiTimeFrameAnalyzer, MultiTimeFrameSignalEnhancer]:
    """

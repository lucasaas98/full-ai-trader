...
class DatabaseTester:
    ...
    def bulk_insert_market_data(self, data_batch: List[Dict[str, Any]]) -> Dict[str, Any]:
...
class TestRedisPubSub:
    ...
    def test_multiple_channels_subscription(self, redis_tester):
        ...
        for expected_channel in channels:
            assert expected_channel in channels_received, f"Missing message from {expected_channel}"
    ...
    def test_high_frequency_publishing(self, redis_tester):
        ...
        print(f"Published {message_count} messages in {publish_duration:.2f}s "
              f"({publish_rate:.2f} msg/sec)")
        print(f"Received {len(messages)} messages")

        # Verify high message throughput
        assert publish_rate > 500, f"Publishing rate too low: {publish_rate:.2f} msg/sec"
        assert len(messages) >= message_count * 0.95, f"Message loss detected: {len(messages)}/{message_count}"
    ...
    def test_message_ordering(self, redis_tester):
        ...
        for i in range(1, len(sequences)):
            assert sequences[i] >= sequences[i-1], f"Message order violated at position {i}"
    ...
    def test_subscriber_resilience(self, redis_tester):
        ...
        assert 'after_restart' in received_phases, "Failed to receive messages after restart"
...
class TestDatabaseOperations:
    ...
    def test_market_data_insertion(self, database_tester, sample_market_data):
        ...
        assert result['inserted_id'] is not None, "No ID returned for inserted market data"
        assert result['duration_ms'] < 100, f"Insert too slow: {result['duration_ms']}ms"

        print(f"Market data inserted with ID {result['inserted_id']} in {result['duration_ms']:.2f}ms")
    ...
    def test_trade_data_insertion(self, database_tester, sample_trade_data):
        ...
        assert result['duration_ms'] < 50, f"Trade insert too slow: {result['duration_ms']}ms"

        print(f"Trade inserted with ID {result['inserted_id']} in {result['duration_ms']:.2f}ms")
    ...
    def test_bulk_market_data_insertion(self, database_tester):
        ...
        assert result['inserted_count'] == batch_size, f"Not all records inserted: {result['inserted_count']}/{batch_size}"
        assert result['rate_per_second'] > 100, f"Bulk insertion rate too low: {result['rate_per_second']:.2f}/sec"

        print(f"Bulk inserted {result['inserted_count']} records at {result['rate_per_second']:.2f}/sec")
    ...
    def test_complex_queries_performance(self, database_tester):
        ...
        for query, max_duration_ms in test_queries:
            ...
            print(f"Query executed in {result['duration_ms']:.2f}ms, returned {result['row_count']} rows")
    ...
    def test_concurrent_database_access(self, database_tester):
        ...
        assert success_rate > 0.95, f"Concurrent access success rate too low: {success_rate*100:.1f}%"
        assert operations_per_second > 50, f"Concurrent operation rate too low: {operations_per_second:.2f}/sec"

        print(f"Concurrent database test: {operations_per_second:.2f} ops/sec, "
...
class TestServiceCommunication:
    ...
    async def test_market_data_flow(self, redis_tester, database_tester, sample_market_data):
        ...
        assert db_result['success'], f"Failed to store market data: {db_result.get('error')}"
    ...
    async def test_trading_signal_workflow(self, redis_tester, database_tester):
        ...
        trade_data = {
            ...
            'execution_time': datetime.fromisoformat(execution_data['execution_time'].replace('Z', '+00:00')),
    ...
    async def test_system_heartbeat_monitoring(self, redis_tester):
        ...
        services = ['data_collector', 'strategy_engine', 'risk_manager', 'trade_executor']
        ...
        messages = redis_tester.get_messages(timeout=3.0)
        assert len(messages) == len(services), f"Expected {len(services)} heartbeats, got {len(messages)}"
        ...
        for service in services:
            assert service in received_services, f"Heartbeat from {service} not received"
...
class TestEndToEndTradingFlow:
    ...
    async def test_complete_trading_cycle(self, redis_tester, database_tester):
        ...
        channels = ['market_data', 'trading_signals', 'risk_checks', 'trade_execution', 'position_updates']
        ...
        expected_channels = ['market_data', 'trading_signals', 'risk_checks', 'trade_execution', 'position_updates']
        received_channels = [msg['channel'] for msg in all_messages]

        for expected_channel in expected_channels:
            assert expected_channel in received_channels, f"Missing message from {expected_channel}"
        ...
        db_trade_data = {
            ...
            'execution_time': datetime.fromisoformat(trade_execution['execution_time'].replace('Z', '+00:00')),
    ...
    async def test_error_propagation_and_recovery(self, redis_tester):
        ...
        error_messages = [msg for msg in all_messages if msg['channel'] == error_channel]
        recovery_messages = [msg for msg in all_messages if msg['channel'] == recovery_channel]

        assert len(error_messages) == len(error_scenarios), "Not all errors propagated"
        assert len(recovery_messages) == len(recovery_actions), "Not all recovery actions propagated"

        print(f"Error propagation test: {len(error_messages)} errors, {len(recovery_messages)} recoveries")
...
class TestDatabaseIntegration:
    ...
    def test_concurrent_read_write_operations(self, database_tester):
        ...
        for worker_id in range(5):
            thread = threading.Thread(target=writer_worker, args=(worker_id, write_results))
            threads.append(thread)
            thread.start()
        ...
        assert successful_writes >= 450, f"Too many write failures: {successful_writes}/500"
        assert successful_reads >= 95, f"Too many read failures: {successful_reads}/100"

        print(f"Concurrent DB test: {successful_writes} successful writes, {successful_reads} successful reads")
    ...
    def test_data_consistency_across_tables(self, database_tester):
        ...
        assert result_row['market_price'] == result_row['trade_price'], "Price mismatch between tables"
        assert result_row['value'] == result_row['trade_price'] * result_row['quantity'], "Value calculation incorrect"
    ...
    def test_database_performance_under_load(self, database_tester):
        ...
        print(f"Database load test: {operations_per_second:.2f} ops/sec, {success_rate*100:.1f}% success rate")
...
class TestDataPipeline:
    ...
    async def test_market_data_pipeline(self, redis_tester, database_tester):
        ...
        all_messages = redis_tester.get_messages(timeout=5.0)
        market_data_messages = [msg for msg in all_messages if msg['channel'] == 'market_data']
        processed_messages = [msg for msg in all_messages if msg['channel'] == 'processed_data']

        expected_market_messages = len(data_sources) * len(symbols)
        assert len(market_data_messages) == expected_market_messages, \
            f"Expected {expected_market_messages} market data messages, got {len(market_data_messages)}"

        assert len(processed_messages) == 5, f"Expected 5 processed messages, got {len(processed_messages)}"
        ...
        for data in published_data[:3]:
            ...
            assert db_result['success'], f"Failed to persist data: {db_result.get('error')}"

        print(f"Data pipeline test: {len(market_data_messages)} raw, {len(processed_messages)} processed messages")
    ...
    async def test_real_time_aggregation(self, redis_tester, database_tester):
        ...
        messages = redis_tester.get_messages(timeout=3.0)
        aggregation_messages = [msg for msg in messages if msg['channel'] == aggregation_channel]

        expected_aggregations = data_points // 10
        assert len(aggregation_messages) == expected_aggregations, \
        ...
        for msg in aggregation_messages:
            ...
            assert agg_data['min_price'] <= agg_data['avg_price'] <= agg_data['max_price']

        print(f"Real-time aggregation test: {len(aggregation_messages)} aggregations processed")
...
class TestSystemResilience:
    ...
    async def test_redis_connection_recovery(self, redis_tester):
        ...
        assert recovery_data['phase'] == 'after_recovery', "Recovery message not received"
    ...
    def test_database_connection_pool_behavior(self, database_tester):
        ...
        try:
            ...
            print(f"Connection pool test: {len(connections)} concurrent connections successful")
    ...
    async def test_message_durability(self, redis_tester):
        ...
        assert queue_length == messages_to_send, f"Expected {messages_to_send} messages, got {queue_length}"
        ...
        for i, consumed in enumerate(consumed_messages):
            ...
            original_message = next((msg for msg in sent_messages if msg['id'] == expected_id), None)
...
class TestServiceHealthMonitoring:
    ...
    def test_health_check_aggregation(self, redis_tester):
        ...
        services = [
            ('data_collector', 'healthy', {'cpu': 25.5, 'memory': 128, 'connections': 5}),
            ('strategy_engine', 'healthy', {'cpu': 45.2, 'memory': 256, 'models_loaded': 3}),
            ('risk_manager', 'healthy', {'cpu': 15.8, 'memory': 64, 'rules_active': 12}),
            ('trade_executor', 'degraded', {'cpu': 60.1, 'memory': 192, 'pending_orders': 25}),
        ...
        messages = redis_tester.get_messages(timeout=3.0)
        assert len(messages) == len(services), f"Expected {len(services)} health reports, got {len(messages)}"
        ...
        assert healthy_services >= 4, f"Too many unhealthy services: {healthy_services}/5 healthy"
        print(f"System health: {healthy_services} healthy, {degraded_services} degraded services")
    ...
    async def test_alert_escalation(self, redis_tester):
        ...
        escalation_messages = [msg for msg in messages if msg['channel'] == escalation_channel]

        assert len(alert_messages) == 4, f"Expected 4 alerts, got {len(alert_messages)}"
        assert len(escalation_messages) == 1, f"Expected 1 escalation, got {len(escalation_messages)}"
...
class TestExternalAPIIntegration:
    ...
    async def test_market_data_api_integration(self, redis_tester):
        ...
        print(f"API integration test: {successful_responses} successful, {failed_responses} failed responses")
...
class TestIntegrationPerformance:
    ...
    async def test_end_to_end_latency(self, redis_tester, database_tester):
        ...
        # Verify all messages received
        all_messages = redis_tester.get_messages(timeout=5.0)
        assert len(all_messages) == 200, f"Expected 200 messages (50 x 4), got {len(all_messages)}"

        # Analyze latency statistics
        avg_latency = statistics.mean(flow_latencies)
        p95_latency = statistics.quantiles(flow_latencies, n=20)[18] if len(flow_latencies) > 1 else 0
        ...
        print(f"End-to-end latency: avg={avg_latency:.2f}ms, p95={p95_latency:.2f}ms, max={max_latency:.2f}ms")
    ...
    async def test_throughput_scalability(self, redis_tester):
        ...
        for target_rate in message_rates:
            ...
            print(f"Throughput test: Target {target_rate}/s, Actual {actual_rate:.2f}/s")
        ...
        for result in throughput_results:
            ...
            assert accuracy >= 0.9, f"Throughput too low for {target}/s: {actual:.2f}/s ({accuracy*100:.1f}%)"
        ...
        assert message_loss_rate < 0.05, f"Message loss rate too high: {message_loss_rate*100:.1f}%"

        print(f"Throughput scalability test completed: {len(final_messages)}/{total_expected} messages received")
...
class IntegrationTestHelper:
    ...
    @staticmethod
    def wait_for_condition(condition_func, timeout_seconds: float = 5.0, check_interval: float = 0.1) -> bool:
    ...
    @staticmethod
    def generate_test_data_batch(count: int, symbol_prefix: str = "TEST") -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def verify_message_integrity(sent_messages: List[Dict], received_messages: List[Dict]) -> bool:
        ...
        for sent, received in zip(sent_messages, received_messages):
            received_data = json.loads(received['data']) if isinstance(received['data'], str) else received['data']
...
def test_integration_test_suite_performance():
    ...
    assert memory_growth < 50, f"Integration tests using too much memory: {memory_growth:.2f}MB"

    print(f"Integration test performance: {duration:.2f}s duration, {memory_growth:.2f}MB memory growth")

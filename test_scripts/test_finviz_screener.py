    ...
    try:
        ...
        if result.data:
            ...
            for i, stock in enumerate(result.data[:5], 1):
                print(f"  {i}. {stock.ticker}: ${stock.price:.2f} | Volume: {stock.volume:,}")
        else:
...
async def test_momentum_screener():
    ...
    try:
        ...
        if result.data:
            print("\nTop momentum stocks:")
            for i, stock in enumerate(result.data[:5], 1):
                print(f"  {i}. {stock.ticker}: ${stock.price:.2f} | Change: {stock.change_percent:.2f}%")
        else:
...
async def test_large_cap_stocks():
    ...
    try:
        ...
        if result.data:
            print("\nLarge cap stocks:")
            for i, stock in enumerate(result.data[:5], 1):
                print(f"  {i}. {stock.ticker}: ${stock.price:.2f} | Market Cap: {stock.market_cap:,}")
        else:

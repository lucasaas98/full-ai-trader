...
class MetricsCollector:
            ...
            try:
                ...
                async with httpx.AsyncClient() as client:
                    response = await client.get(
                        f"http://{service_name}:{port}/health",
                        timeout=5.0
                    )
    ...
    async def collect_portfolio_metrics(self):
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                if portfolio_row:
                    self.metrics.portfolio_total_value.set(portfolio_row['total_value'] or 0)
                    self.metrics.portfolio_cash.set(portfolio_row['cash'] or 0)
                    self.metrics.portfolio_buying_power.set(portfolio_row['buying_power'] or 0)
                    self.metrics.portfolio_daily_pnl.set(portfolio_row['daily_pnl'] or 0)
                    self.metrics.portfolio_cumulative_pnl.set(portfolio_row['cumulative_pnl'] or 0)
                    ...
                    if portfolio_row['sharpe_ratio']:
                        self.metrics.portfolio_sharpe_ratio.set(portfolio_row['sharpe_ratio'])
                    if portfolio_row['sortino_ratio']:
                        self.metrics.portfolio_sortino_ratio.set(portfolio_row['sortino_ratio'])
    ...
    async def collect_system_metrics(self):
        ...
        try:
            ...
            if os.path.exists('/app/data'):
                ...
                self.metrics.system_disk_usage.labels(mount_point='/app/data').set(data_disk.used)
    ...
    async def collect_database_metrics(self):
        ...
        try:
            ...
            async with self._db_pool.acquire() as conn:
                ...
                self.metrics.database_connections_active.labels(database='postgres').set(active_connections)

                # Database size
                db_size = await conn.fetchval(
                    "SELECT pg_database_size(current_database())"
                )
                self.metrics.database_size_bytes.labels(database='postgres').set(db_size or 0)
    ...
    def record_api_request(self, service: str, method: str, endpoint: str,
    ...
    def record_trade_execution(self, symbol: str, side: str, strategy: str,
                             latency: float, pnl: Optional[float] = None):
    ...
    def record_signal_generation(self, symbol: str, strategy: str,
    ...
    def record_market_data_collection(self, symbol: str, timeframe: str,
                                    source: str, success: bool, error_type: Optional[str] = None):
        ...
        if success:
            ...
            self.metrics.market_data_last_update_timestamp.labels(symbol=symbol).set(time.time())
        else:
    ...
    def record_database_operation(self, query_type: str, duration: float,
        ...
        self.metrics.database_query_duration.labels(query_type=query_type).observe(duration)
    ...
    def record_redis_operation(self, operation: str, duration: float, success: bool):
        ...
        self.metrics.redis_operation_duration.labels(operation=operation).observe(duration)
    ...
    def update_portfolio_metrics(self, portfolio_state: PortfolioState):
        ...
        if portfolio_state.total_equity > 0:
            ...
            for position in portfolio_state.positions:
                concentration = float(position.market_value) / float(portfolio_state.total_equity)
                self.metrics.position_size_percentage.labels(
                    symbol=position.symbol
                ).set(concentration)
                max_concentration = max(max_concentration, concentration)
...
class AlertManager:
    ...
    def __init__(self, config: Config, metrics_collector: MetricsCollector):
        ...
        if (hasattr(config, 'notifications') and
            config.notifications.gotify_url and
            config.notifications.gotify_token):
            self.gotify_client = GotifyClient(config.notifications.gotify_url, config.notifications.gotify_token)
    ...
    async def check_alerts(self):
        ...
        for rule in self.alert_rules:
            try:
                ...
                self.logger.error(f"Failed to evaluate alert rule {rule.get('name', 'unknown')}: {e}")

...
class ExportService:
    ...
    async def export_tradenote_format(self, request: ExportRequest) -> str:
        ...
        try:
            ...
            if tradenote_data:
                ...
                headers = ["Date", "Time", "Symbol", "Side", "Quantity", "Price", "Commission", "PnL", "Strategy", "Notes"]
    ...
    async def export_performance_report(self, request: ExportRequest) -> str:
        ...
        try:
            ...
            for row in strategy_rows:
                ...
                strategy_win_rate = (row['wins'] / strategy_trades * 100) if strategy_trades > 0 else 0
        ...
        try:
            ...
            for row in rows:
                ...
                tax_lots.append({
                    ...
                    "quantity": float(row['quantity']),
                    "cost_basis": float(row['opening_price']) * float(row['quantity']),
                    "sale_proceeds": float(row['closing_price']) * float(row['quantity']),
            ...
            # Calculate summary
            total_short_term_gain = sum(lot['gain_loss'] for lot in tax_lots if lot['term'] == 'Short')
            total_long_term_gain = sum(lot['gain_loss'] for lot in tax_lots if lot['term'] == 'Long')
            total_commission = sum(lot['commission'] for lot in tax_lots)

            tax_report = {
                "tax_year": request.start_date.year if request.start_date else datetime.now().year,
                "summary": {
    ...
    async def export_audit_trail(self, request: ExportRequest) -> str:
        ...
        try:
            ...
            if request.start_date:
                ...
                query = query.replace("UNION ALL", f"AND timestamp >= ${param_count} UNION ALL")
                params.append(request.start_date)
                param_count += 1
            ...
            if request.end_date:
                ...
                query = query.replace("UNION ALL", f"AND timestamp <= {end_param} UNION ALL")
                params.append(request.end_date)
                param_count += 1
        ...
        try:
            ...
            for row in risk_rows:
                risk_metrics.append({
                    ...
                    "risk_score": float(row['pnl_volatility'] or 0) / abs(float(row['avg_pnl'])) if row['avg_pnl'] else 0
                })
    ...
    async def create_export_archive(self, filepaths: List[str], archive_name: str) -> str:

...
def create_plane_task(api_key: str, workspace: str, project_id: str, plane_instance: str, task: Dict) -> bool:
    """Create a single task in Plane using requests."""
    url = f"{plane_instance}/api/v1/workspaces/{workspace}/projects/{project_id}/issues/"
...
def main():
    ...
    for i, task in enumerate(TASKS, 1):
        ...
        success = create_plane_task(api_token, workspace, project_id, plane_instance, task)
        if success:
            successful_tasks += 1
        else:
            failed_tasks += 1

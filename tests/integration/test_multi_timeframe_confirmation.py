    ...
    def generate_ohlcv_data(
        ...
        for i in range(periods):
            ...
            open_price = current_price * (
                1 + np.random.uniform(-daily_vol / 2, daily_vol / 2)
            )
            close = current_price
            ...
            timestamp = current_time - timedelta(
                minutes=interval_minutes * (periods - i)
            )
...
class MultiTimeFrameAnalyzer:
    ...
    def analyze_timeframe(self, data: pd.DataFrame, timeframe: str) -> TimeFrameSignal:
        ...
        # Momentum
        price_change = (
            (current_price - closes[-10]) / closes[-10] if len(closes) >= 10 else 0
        )
    ...
    async def confirm_signal(
        ...
        support_ratio = (
            len(supporting_timeframes) / total_signals if total_signals > 0 else 0
        )
        ...
        if supporting_timeframes:
            supporting_signals = [
                s for s in timeframe_signals if s.timeframe in supporting_timeframes
            ]
            ...
            total_weight = sum(
                self.timeframe_weights.get(s.timeframe, 0.2) for s in supporting_signals
            )
            overall_confidence = (
        ...
        return MultiTimeFrameConfirmation(
            ...
            risk_factors=[],
            optimal_entry_timeframe=(
                supporting_timeframes[0] if supporting_timeframes else None
            ),
        )
    ...
    def should_allow_entry(
        ...
        if current_level < required_level:
            return (
                False,
                f"Insufficient confirmation strength: {confirmation.confirmation_strength.value}",
            )
        ...
        return (
            True,
            f"Multi-timeframe confirmation passed: {confirmation.confirmation_strength.value}",
        )
...
class TestMultiTimeFrameConfirmation:
    ...
    async def test_bullish_alignment_confirmation(
        ...
        for tf in timeframes:
            multi_tf_data[tf] = mock_data_generator.generate_ohlcv_data(
                periods=60, timeframe=tf, trend="bullish", volatility=0.015
            )
        ...
        assert confirmation.confirmation_strength in [
            ConfirmationStrength.STRONG,
            ConfirmationStrength.VERY_STRONG,
        ]
        assert confirmation.alignment_type in [
            TimeFrameAlignment.BULLISH_ALIGNMENT,
            TimeFrameAlignment.MIXED_BULLISH,
        ]
    ...
    async def test_bearish_alignment_confirmation(
        ...
        for tf in timeframes:
            multi_tf_data[tf] = mock_data_generator.generate_ohlcv_data(
                periods=60, timeframe=tf, trend="bearish", volatility=0.015
            )
        ...
        assert confirmation.alignment_type in [
            TimeFrameAlignment.BEARISH_ALIGNMENT,
            TimeFrameAlignment.MIXED_BEARISH,
        ]
        assert confirmation.overall_confidence > 60.0
        assert len(confirmation.supporting_timeframes) >= len(timeframes) * 0.6
    ...
    async def test_day_trading_vs_swing_trading_thresholds(
        ...
        swing_allow_entry, swing_reason = swing_analyzer.should_allow_entry(
            swing_confirmation
        )
    ...
    async def test_insufficient_timeframe_data(self, swing_analyzer):
        ...
        assert confirmation.confirmation_strength in [
            ConfirmationStrength.WEAK,
            ConfirmationStrength.VERY_WEAK,
            ConfirmationStrength.MODERATE,
        ]
    ...
    async def test_timeframe_signal_analysis(self, swing_analyzer, mock_data_generator):
        ...
        bullish_data = mock_data_generator.generate_ohlcv_data(
            periods=50, timeframe="1h", trend="bullish", volatility=0.01
        )
        ...
        assert tf_signal.signal_type in [
            SignalType.BUY,
            SignalType.HOLD,
    ...
    def test_confirmation_strength_ordering(self):
        ...
        assert (
            strength_values[ConfirmationStrength.VERY_STRONG]
            > strength_values[ConfirmationStrength.WEAK]
        )
        assert (
            strength_values[ConfirmationStrength.STRONG]
            > strength_values[ConfirmationStrength.MODERATE]
        )
    ...
    async def test_performance_under_load(self, swing_analyzer, mock_data_generator):
        ...
        for symbol in symbols:
            ...
            for tf in timeframes:
                multi_tf_data[tf] = mock_data_generator.generate_ohlcv_data(
                    periods=50, timeframe=tf, trend="bullish"
                )
            ...
            assert confirmation.primary_signal in [
                SignalType.BUY,
                SignalType.SELL,
                SignalType.HOLD,
            ]
        ...
        assert (
            execution_time < 5.0
        ), f"MTF confirmation took too long: {execution_time:.2f}s"
